...
- _Keeping it as a required field but ignoring its value_: Still costs output tokens and encourages the model to hallucinate, which is a bad pattern.
- _Removing it entirely from the system_: Violates the need for cryptographic evidence binding in the research protocol.
